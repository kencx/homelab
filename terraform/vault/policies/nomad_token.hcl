# Allow creating tokens under "nomad_cluster" token role.
path "auth/token/create/nomad_cluster" {
  capabilities = ["update"]
}

# Allow looking up "nomad_cluster" token role.
path "auth/token/roles/nomad_cluster" {
  capabilities = ["read"]
}

# Allow looking up the token passed to Nomad to validate
# the token has the proper capabilities.
